/*
 * UDP Bypass (ESP encaps) - Standalone x86/x64
 * by c0re_x64 for 0v.lol // nigga
 * usage: sudo ./udp_bypass <ip> <port> <threads> <duration> [--spoof][--burst N][--size N]
 */
...
static struct udpbypass_config config;
...
// nigga udp bypass
static void* udp_bypass_thread(void *arg) {
...
int main(int argc, char **argv) {
    if (argc < 5) { printf("udp_bypass <ip> <port> <threads> <duration> [--spoof][--burst N][--size N]\n"); return 1; }
